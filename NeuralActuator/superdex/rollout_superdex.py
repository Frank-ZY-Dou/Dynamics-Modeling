"""Roll out a task with the SuperDex backend (the differentiable SuperDex fork's
Backward-Euler dynamics) and dump a trajectory. The predicted joint path sim_q is
advanced by SuperDexBackendTorch, not MJX; the step logic mirrors the public
evaluate_actuator.py single_step (feature update from sim state, torque clip,
gripper clamp, qvel clip, history roll). Dumps npz compatible with the paper
renderer and with render_superdex.py.

Dynamics-rollout mode: sim_q comes from SuperDex.
...
from superdex_backend_torch import SuperDexBackendTorch
...
def main():
    ...
    ap.add_argument("--device", default="cuda", help="device of the network; the engine runs on the CPU")
    args = ap.parse_args()
    ...
    be = None if args.force_only else SuperDexBackendTorch(1, args.data_dt, args.sim_step_size,
                                                           device=str(dev))
    ...
    print(f"{args.task_name}: {'force-only' if args.force_only else 'superdex dynamics'} "
