"""Roll out a task with the Newton backend (genuine Newton dynamics) and dump a
trajectory. The predicted joint path sim_q is advanced by NewtonBackendTorch,
...
roll). Dumps npz compatible with the MuJoCo and Newton renderers.
...
def main():
    ...
    be = None if args.force_only else NewtonBackendTorch(1, args.data_dt, args.sim_step_size,
                                                         device=str(dev))
    ...
    print(f"{args.task_name}: {'force-only' if args.force_only else 'Newton dynamics'} "
